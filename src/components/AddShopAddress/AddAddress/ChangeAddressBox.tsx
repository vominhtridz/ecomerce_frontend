...
import { ChangeAddressBoxTypes } from "../../../typescript/AddressTypes"
import { CityAndDistrictsItems } from "..//index"
import NavLinkAddress from "./NavLinkAddress"

const ChangeAddressBox: FC<ChangeAddressBoxTypes> = ({
  AddressWard,
  setWard,
  setVisiAddress,
  RouteAddress,
  ...
  const getWard = AddressWard?.wards.map((warditem, index) => {
  ...
  const getDistrict = AddressDisAndWardAndCity?.districts.map((districtItem, index) => {
    function handleSetLocation() {
  ...
  return (
    <div>
      <div
        ...
        className={`
               absolute z-50 transition duration-500 bottom-full border-t border-r border-l border-slate-300  pb-14 left-0 right-0
                h-[20rem] bg-white rounded-sm shadow-lg py-6 px-3
            `}
      >
        <NavLinkAddress
          Ward={Ward}
        ...
        />
        {booleanProvince && (
          <div className='h-full overflow-y-auto my-2 text-slate-500 flex items-baseline flex-wrap '>
            {getCityAndDistrictsItems}
          </div>
        )}
        {booleanDistrict && (
          <div className='h-full overflow-y-auto my-2 text-slate-500  flex items-baseline flex-wrap '>
        ...
        {booleanWard && (
          <div className='h-full overflow-y-auto my-2 text-slate-500 flex items-baseline clear-both flex-wrap '>
            {getWard}
          </div>
        )}
      </div>
