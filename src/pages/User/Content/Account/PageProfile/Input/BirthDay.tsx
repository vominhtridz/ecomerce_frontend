import { birthdayType } from "@/typescript/UserTypes"
import { FC } from "react"
const Days = Array.from({ length: 31 }, (_, i) => i + 1)
const Years = Array.from({ length: 100 }, (_, i) => new Date().getFullYear() - i)
const Months = Array.from({ length: 12 }, (_, i) => i + 1)
const BirthdayField: FC<birthdayType> = ({ day, month, year, ChangeDay, ChangeMonth, ChangeYear }) => {
  // --------------------------- DAY--------------------
  const getYear = () =>
  ...
  return (
    <div className='flex items-center py-2 ml-8'>
      <label className='text-sm whitespace-nowrap mr-2'>Ngày Sinh</label>
      <select
        value={day}
        onChange={ChangeDay}
        name='day'
