...
const NameAndAddress: FC<NameAndAddressTypes> = ({
  ...
  const lengthNameSeller = nameShopInput.length
  ...
  return (
    <div className=''>
      <nav className='flex items-center  py-2'>
        ...
        <div className='w-[26rem] relative'>
          ...
          <div className='flex text-sm  absolute right-2 top-1.5 items-center text-slate-500'>
            <p className='text-sm'>{lengthNameSeller}</p>/30
          </div>
        </div>
      </nav>
      <nav className='flex  py-4'>
        ...
        {sellerAddress ? RenderSellerAddress() : <ChangeBtn handleChange={handleChangeAdress} />}
      </nav>
    </div>
  )
}
